...
export async function GET(request: Request) {
  ...
  const provider = (url.searchParams.get('provider') ?? 'indeed').toLowerCase();
  ...
  try {
    ...
    const xml = buildAggregatorXmlFeed(provider === 'jooble' ? 'jooble' : 'indeed', transformedJobs, SITE_URL);
